...
SimpleExpression]. assignment = ident selector ":=" expression. ActualParameters
= "(" [expression {"," expression}] ")" . ProcedureCall = ident selector
[ActualParameters]. IfStatement = "IF" expression "THEN" StatementSequence
...
declarations = ["CONST" {ident "=" expression ";"}] ["TYPE" {ident "=" type
";"}] ["VAR" {IdentList ":" type ";"}] {ProcedureDeclaration ";"}. module =
...
std::unique_ptr<const Node> Parser::parse() {
...
void Parser::requireToken(const TokenType &type) {
  ...
  if (token->getType() != type) {
    logger_->error(token->getPosition(),
                   "Expected " + type_str(type) + " but got " +
...
std::string Parser::ident() {
  ...
  if (id->getType() != TokenType::const_ident) {
    logger_->error(id->getPosition(), "Expected identifier but got " +
                                          (std::stringstream() << *id).str() +
                                          ".");
    // TODO: terminate
  }
...
const Node *Parser::module() {
  ...
  if (moduleName != moduleName2) {
    logger_->error(namePos, "Expected equal module names but got " +
                                moduleName + " and " + moduleName2 + ".");
    // TODO: terminate
  }
...
const Node *Parser::simple_expression() {
  ...
  while (next == TokenType::op_plus || next == TokenType::op_minus ||
         next == TokenType::op_or) {
...
const Node *Parser::term() {
  ...
  auto next = scanner_->peekToken()->getType();
  while (next == TokenType::op_times || next == TokenType::op_div ||
         next == TokenType::op_mod || next == TokenType::op_and) {
...
const Node *Parser::factor() {
  ...
  if (next->getType() == TokenType::const_ident) {
    ...
    const auto numToken = dynamic_cast<const IdentToken *>(num.get());
    if (numToken == nullptr)
      throw new std::exception("Failed IdentToken cast");
    ...
    logger_->error(next->getPosition(),
                   "Expected Identifier, Number, lparen or ~ but got " +
...
const Node *Parser::type() {
  ...
  if (next->getType() == TokenType::const_ident) {
    ...
    logger_->error(next->getPosition(),
                   "Expected Identifier, ARRAY or RECORD but got " +
                       (std::stringstream() << *next).str() + ".");
    // TODO: terminate
  }
  return nullptr;
...
const Node *Parser::statement() {
  ...
  if (next->getType() == TokenType::const_ident) {
    ...
    logger_->error(next->getPosition(),
                   "Expected Identifier, IF or WHILE but got " +
                       (std::stringstream() << *next).str() + ".");
    // TODO: terminate
  }
  return nullptr;
